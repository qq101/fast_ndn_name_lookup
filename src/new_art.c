...
static art_node** find_child_port(art_node *n, unsigned char c, int  *port) {
	int i, mask, bitfield;
	union {
		art_node4 *p1;
		art_node16 *p2;
	...
	case NODE4:
		...
		for (i = 0; i < n->num_children; i++) {
			//增加取port功能
			if ((c == '/' ) && (p.p1->keys[0] == '\0') && (flag_4 == 0))
			{
				*port = (uintptr_t)(((art_leaf*)(LEAF_RAW(p.p1->children[0])))->value);
				flag_4 = 1;
			}
			if (p.p1->keys[i] == c)
				return &p.p1->children[i];
		...
		case NODE16:
			p.p2 = (art_node16*)n;
			int flag_16 = 0;
			if ((p.p2->keys[0] == '\0' ) && (c == '/') && (flag_16 == 0))
			{
				/*art_node ** tmp_root_16;
				tmp_root_16 = (&p.p2->children[0]);
				art_node *tmp_leaf_16;
				tmp_leaf_16 = LEAF_RAW(*tmp_root_16);
				*port = (uintptr_t)(((art_leaf*)tmp_leaf_16)->value);*/
				*port = (uintptr_t)(((art_leaf*)(LEAF_RAW(p.p2->children[0])))->value);
				flag_16 = 1;
			}
	...
	case NODE48:
		...
		if (i0 && (c == '/') && (flag_48 == 0))
		{
			*port = (uintptr_t)(((art_leaf*)(LEAF_RAW(p.p3->children[i0-1])))->value);
			flag_48 = 1;
		}
	...
	case NODE256:
		...
		//如果存在\0结尾的串
		int flag_256 = 0;
		if (p.p4->children['\0'] && (c == '/') && (flag_256 == 0))
		{
			*port = (uintptr_t)(((art_leaf*)(LEAF_RAW(p.p4->children['\0'])))->value);
			flag_256 = 1;
		}
...
static int leaf_matches(const art_leaf *n, const unsigned char *key, int key_len, int depth) {
	...
	return memcmp(n->key, key, key_len - 1);
}
...
int* art_search(const art_tree *t, const unsigned char *key, int key_len) {
	...
	while (n) {
		...
		if (IS_LEAF(n)) {
			...
			// new add modify
			int tmp_key = ((art_leaf*)n)->key_len - 1;
			if ((((art_leaf*)n)-> key[tmp_key] == '\0') && key[tmp_key] == '/')
			{
				if (memcmp(((art_leaf*)n)->key, key, tmp_key) == 0)
		...
		// Recursively search
		child = find_child_port(n, key[depth], &port);
		n = (child) ? *child : NULL;
		depth++;
	}
	return NULL;
